...
UCameraComponent* ANP4PlayerBase::GetCameraComponent()
{
	return m_OurCamera;
}

USpringArmComponent* ANP4PlayerBase::GetSpringArmCompoennt()
{
...
void ANP4PlayerBase::ActionAttack()
{
	...
	if (CurState != eCharacterState::eAttack &&
		...
		CurState != eCharacterState::eSkilling)
	{
		/* MathManager를 이용하여 방향을 잡는다. */
		ANP4GameState* pGameState = Cast<ANP4GameState>(GetWorld()->GetGameState());
		
		if (pGameState)
		{
			FRotator fResultRot;
			bool bFind = pGameState->GetMathManager()->TraceNearEnemyObject_ByBox(
				this, m_pTargetEnemy,20,FVector(20.f, 20.f, 40), fResultRot);

			if (bFind == true)
			{
				SetActorRotation(fResultRot);
			}
		}
